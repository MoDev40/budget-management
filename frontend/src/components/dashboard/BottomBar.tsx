    ...
    <div className="w-full md:px-4 mt-5 space-y-10 px-10">
      <div className="flex flex-row  md:px-2 items-center justify-between">
        <div className="flex flex-col  space-y-1">
          <Button className="flex flex-row space-x-3" variant="outline"><LucideBadgeDollarSign/><span>${isFetching ? "...." : budget?.isBudgetExists?.amount}</span><p className="hidden md:block">Your Budget/Month</p></Button>
          <p className="font-thin text-sm">Ends: {new Date(budget?.isBudgetExists?.endDate).toDateString()}</p>
        </div>
        <div className="flex my-2 flex-col space-y-1">
          <Button className="flex flex-row space-x-3" variant="outline"><LucideBadgeDollarSign/><span>${isFetching ? "...." : budget?.isBudgetExists?.balance[0].amount }</span><p className="hidden md:block">Your Balance/Month</p></Button>
          <p className="font-thin text-sm">Ends: {new Date(budget.isBudgetExists.balance[0]?.toDate).toDateString()}</p>
        </div>
      </div>
